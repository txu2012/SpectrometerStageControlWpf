...
using SpectrometerStageControlWpf;
...
namespace WPFSurfacePlot3D
{
    ...
    class SurfacePlotModel : INotifyPropertyChanged
    {
        ...
        {
            Title = "Spectrum";
            XAxisLabel = "Wavelengths";
            YAxisLabel = "Delays";
            ZAxisLabel = "Intensities";
        ...
        public void PlotData(List<FrogData> data)
        {
            if (data.Count <= 0) return;

            double[] wavelengths = data[0].SpectrumData.Wavelengths;
            int waves = wavelengths.Length;
            int delays = data.Count;

            double[,] arrayOfPoints = new double[waves, delays];
            for (int d = 0; d < delays; d++)
            {
                var intensities = data[d].SpectrumData.Intensities;
                for (int w = 0; w < waves; w++)
                {
                    arrayOfPoints[w, d] = intensities[w];
                }
            }

            PlotData(arrayOfPoints);

            /*Point3D[,] newDataArray = new Point3D[waves, delays];
            for (int z = 0; z < delays; z++)
            {
                var frog = data[z];
                for (int x = 0; x < waves; x++)
                {
                    Point3D point = new Point3D(x, z, frog.SpectrumData.Intensities[x]);
                    newDataArray[x,z] = point;
                }
            }
